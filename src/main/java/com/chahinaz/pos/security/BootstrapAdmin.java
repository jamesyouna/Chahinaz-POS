...
public class BootstrapAdmin {
  @Bean CommandLineRunner createFirstAdmin(EmployeeRepository employees, PasswordEncoder encoder, Environment env, AuditService audit, TransactionTemplate transactions) {
    return args -> transactions.executeWithoutResult(status -> {
      ...
      Employee admin = employees.saveAndFlush(new Employee(username.trim(), "Store Administrator", encoder.encode(password), Role.ADMIN));
      audit.record(admin.id, "EMPLOYEE_CREATED", "employee", admin.id.toString(), null, Role.ADMIN.name());
    });
  }
}
